...
/* Compares the first n characters of string1 to the first num characters of string2.
* The comparison is performed character by character. If a character that is not equal
* in both strings is found the function ends and returns a value that determines which of
* them was greater. */
...
/* A pointer to the next token from stringp, as delimited by delim. 
...
char * strsep(char **stringp, const char *delim)
{
 ...
 /* Returns a random number without a seed. */
u_long random()
{
 ...
 /* Returns a random number with the specified seed. */
...
* Assume "a" is the input integer with less than 18 digits,
* and "s" is a string pointer for output. */
...
/* Converts an integer to a Hexidecimal character string. */
void int2hex(int a, char *s)
{
	unsigned char size = sizeof(int)*2;		//2 hex digits for each byte(8bits)
	char *hlookup = "0123456789abcdef";	//lookup table stores the hex digits into their
										//corresponding index.
	int temp = a;						//temporary place holder for _num of the same type
										//as _num
	if(a<0)						
	{									
		s[0]='-';					//if _num is negative make the sign negative
		a *= -1;					//and make _num positive to clear(zero) the sign bit
	}
	else
		s[0]=' ';
	
	char mask = 0x000f;				//mask will clear(zero) out all the bits except lowest 4	 which represent a single hex digit
	char x=0;
	for(;x<size;x++)
	{	
		temp = a;					//temp is assigned _num each time
		temp >>=(4*x);					//shift it in multiples of 4 bits each time
		temp &= mask;					//mask the integer so it will only have lowest 4 bits
		s[size-x]=hlookup[temp];
	}						
	s[size+1]= 0;				//the last element will store a null
}

/* Converts a Hexidecimal character string to a nibble (4 bit). */
char hex2nibble(char n)
{
...
/* Converts a Hexidecimal character string to an integer. */
int hex2int (char *s)
{
	int j = 0;
	char *p = (char *)s;
...
/* Converts a character string representing an decimal number
* to an actual integer. */
int str2int(char* digit, int* result) 
{
   *result = 0;
...
/* Kernel print function, resembles printf(...), but does not support all the options. */
void kprintf(const char* fmt, ...)
{
	int c;					/* next character in fmt */
	int d;
	unsigned long u = 0;				/* hold number argument */
	int base = 0;					/* base of number arg */
	...
	putch(END_OF_KMESS);				/* terminate output */
	va_end(argp);					/* end variable arguments */
}

/* Bit masking function, Finds the first set bit in a mask.*/
int ffs(int mask)
{
	 int bit;
...
/* Bit masking function, Finds the first set bit in a (long) mask.*/
int ffsl(long mask)
{
	int bit;
...
/* Bit masking function, Finds the last set bit in a mask.*/
int fls(int mask)
{
	int bit;
...
/* Bit masking function, Finds the last set bit in a (long) mask.*/
int flsl(long mask)
{
	int bit;
